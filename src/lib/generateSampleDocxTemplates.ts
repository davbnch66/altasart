...
import { TEMPLATE_VARIABLES, type DocumentType } from "./docxTemplateEngine";
...
function labelValue(label: string, varKey: string): Paragraph {
  return new Paragraph({
    children: [
      new TextRun({ text: `${label} : `, bold: true, size: 20, font: "Calibri" }),
      new TextRun({ text: `{{${varKey}}}`, size: 20, font: "Calibri", color: "0070C0" }),
    ],
    spacing: { after: 60 },
  });
}
...
// ── Devis template ──
function buildDevisDoc(): Document {
  return new Document({
    sections: [{
      properties: {
      ...
      headers: {
        default: new Header({
          children: [
            new Paragraph({
              children: [
                new TextRun({ text: "{{company_name}}", bold: true, size: 24, color: BRAND_COLOR, font: "Calibri" }),
                new TextRun({ text: "  |  {{company_phone}}  |  {{company_email}}", size: 18, color: "666666", font: "Calibri" }),
              ],
            }),
          ],
        }),
        ...
        default: new Footer({
          children: [
            new Paragraph({
              ...
              children: [
                new TextRun({ text: "{{company_name}} — SIRET {{company_siret}} — {{company_address}}", size: 16, color: "999999", font: "Calibri" }),
              ],
            }),
            new Paragraph({
              alignment: AlignmentType.CENTER,
      ...
      children: [
        ...
        infoTable([
          ...
          ["Code postal", "client_postal_code"],
          ["Ville", "client_city"],
          ["Email", "client_email"],
        ]),
        spacer(),
        ...
        infoTable([
          ["Date", "devis_date"],
          ["Validité", "devis_valid_until"],
          ["Objet", "devis_objet"],
          ["Dossier", "dossier_title"],
          ["Adresse chantier", "dossier_address"],
        ]),
        spacer(),
        ...
        sectionTitle("NOTES"),
        new Paragraph({
          children: [new TextRun({ text: "{{devis_notes}}", size: 20, font: "Calibri", italics: true })],
          spacing: { before: 100 },
        }),
      ],
    }],
...
// ── Facture template ──
function buildFactureDoc(): Document {
  return new Document({
    sections: [{
      properties: {
        page: { margin: { top: 1000, right: 1000, bottom: 1000, left: 1000 } },
      ...
      children: [
        ...
        infoTable([
          ["Date", "facture_date"],
          ["Échéance", "facture_due_date"],
          ["Devis associé", "devis_code"],
        ...
        sectionTitle("NOTES"),
        new Paragraph({
          children: [new TextRun({ text: "{{facture_notes}}", size: 20, font: "Calibri", italics: true })],
          spacing: { before: 100 },
        }),
      ],
    }],
...
// ── Visite template ──
function buildVisiteDoc(): Document {
  return new Document({
    sections: [{
      properties: {
        page: { margin: { top: 1000, right: 1000, bottom: 1000, left: 1000 } },
        ...
        default: new Header({
          children: [
            new Paragraph({
              children: [
                new TextRun({ text: "{{company_name}}", bold: true, size: 24, color: BRAND_COLOR, font: "Calibri" }),
                new TextRun({ text: "  |  SIRET {{company_siret}}", size: 18, color: "666666", font: "Calibri" }),
              ],
            }),
          ],
        }),
      ...
      footers: {
        default: new Footer({
          children: [
            new Paragraph({
              alignment: AlignmentType.CENTER,
              children: [new TextRun({ text: "{{company_name}} — {{company_address}} — {{company_phone}}", size: 16, color: "999999", font: "Calibri" })],
            }),
          ],
        }),
      },
      children: [
        ...
        infoTable([
          ["Volume", "visite_volume"],
          ["Distance", "visite_distance"],
        ]),
        spacer(),
        sectionTitle("INSTRUCTIONS"),
        new Paragraph({
          children: [new TextRun({ text: "{{visite_instructions}}", size: 20, font: "Calibri", italics: true })],
          spacing: { before: 100 },
        }),
      ],
    }],
...
// ── BT Report template ──
function buildBTReportDoc(): Document {
  return new Document({
    sections: [{
      properties: {
        page: { margin: { top: 1000, right: 1000, bottom: 1000, left: 1000 } },
        ...
        default: new Header({
          children: [
            new Paragraph({
              children: [
                new TextRun({ text: "{{company_name}}", bold: true, size: 24, color: BRAND_COLOR, font: "Calibri" }),
                new TextRun({ text: "  |  {{company_phone}}", size: 18, color: "666666", font: "Calibri" }),
              ],
            }),
          ],
        }),
      ...
      children: [
        ...
        sectionTitle("NOTES"),
        new Paragraph({
          children: [new TextRun({ text: "{{bt_notes}}", size: 20, font: "Calibri", italics: true })],
          spacing: { before: 100 },
        }),
      ],
    }],
...
const DOC_BUILDERS: Record<DocumentType, () => Document> = {
  devis: buildDevisDoc,
...
export async function downloadSampleTemplate(docType: DocumentType): Promise<void> {
  const doc = DOC_BUILDERS[docType]();
  const blob = await Packer.toBlob(doc);
  saveAs(blob, DOC_FILENAMES[docType]);
}

export async function generateSampleTemplateBlob(docType: DocumentType): Promise<{ blob: Blob; fileName: string }> {
  const doc = DOC_BUILDERS[docType]();
  const blob = await Packer.toBlob(doc);
  return { blob, fileName: DOC_FILENAMES[docType] };
}
